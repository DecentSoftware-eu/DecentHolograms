...
public class NMS_1_9 extends NMS {
    ...
    private static final ReflectMethod ENUM_ITEM_SLOT_A_METHOD;
    ...
    static {
        ...
        ENUM_ITEM_SLOT_A_METHOD = new ReflectMethod(ENUM_ITEM_SLOT_CLASS, "a", String.class);
    ...
    public void showFakeEntityArmorStand(Player player, Location location, int entityId, boolean invisible, boolean small, boolean clickable) {
        ...
        showFakeEntityLiving(player, location, Common.SERVER_VERSION.isBefore(Version.v1_13_R1) ? 30 : 1, entityId, dataWatcher);
    ...
    public void showFakeEntityItem(Player player, Location location, ItemStack itemStack, int entityId) {
        ...
        if (Common.SERVER_VERSION.equals(Version.v1_12_R1)) {
            DATA_WATCHER_REGISTER_METHOD.invoke(dataWatcher, DWO_ITEM, java.util.Optional.ofNullable(nmsItemStack));
        } else if (Common.SERVER_VERSION.equals(Version.v1_11_R1) || Common.SERVER_VERSION.isAfter(Version.v1_12_R1)) {
        ...
        showFakeEntity(player, location, Common.SERVER_VERSION.isBefore(Version.v1_13_R1) ? 2 : 41, entityId);
        sendPacket(player, PACKET_ENTITY_METADATA_CONSTRUCTOR.newInstance(entityId, dataWatcher, true));
    ...
    public void helmetFakeEntity(Player player, ItemStack itemStack, int entityId) {
        Validate.notNull(player);
        Validate.notNull(itemStack);
        Object nmsItemStack = CRAFT_ITEM_NMS_COPY_METHOD.invoke(itemStack);
        Object packet = PACKET_ENTITY_EQUIPMENT_CONSTRUCTOR.newInstance(entityId, ENUM_ITEM_SLOT_A_METHOD.invokeStatic("head"), nmsItemStack);
    ...
    public void attachFakeEnity(Player player, int vehicleId, int entityId) {
        ...
        ReflectionUtil.setFieldValue(packet, "b", new int[] {vehicleId});
        sendPacket(player, packet);
    }
