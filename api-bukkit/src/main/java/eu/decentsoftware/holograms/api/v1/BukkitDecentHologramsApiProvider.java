 ...
 * !! INTERNAL USE ONLY !!
 * <p>
 * Use {@link BukkitDecentHologramsApi#getInstance(Plugin)} to get an instance of the API.
 *
 * @author d0by
 * @see BukkitDecentHologramsApi#getInstance(Plugin)
 * @since 2.10.0
 */
@ApiStatus.Internal
abstract class BukkitDecentHologramsApiProvider {
    ...
    private static BukkitDecentHologramsApiProvider implementation;

    @Contract(pure = true)
    @ApiStatus.Internal
    static BukkitDecentHologramsApiProvider getImplementation(@NotNull Plugin plugin) {
        if (BukkitDecentHologramsApiProvider.implementation == null) {
            logUninitializedAccessError(plugin);
            throw new IllegalStateException(UNINITIALIZED_ERROR_MESSAGE);
        }
        return BukkitDecentHologramsApiProvider.implementation;
    }
    ...
    @ApiStatus.Internal
    static void setImplementation(BukkitDecentHologramsApiProvider implementation) {
        if (BukkitDecentHologramsApiProvider.implementation != null) {
            throw new IllegalStateException("DecentHolograms API is already initialized.");
        }
        BukkitDecentHologramsApiProvider.implementation = implementation;
    }

    abstract BukkitDecentHologramsApi getApi(@NotNull Plugin plugin);
