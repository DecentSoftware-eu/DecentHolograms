...
public final class ConverterCommon {
    ...
    public static void createHologram(ConvertorResult convertorResult, String name, Location location, List<String> lines, HologramManager hologramManager) {
        if (hologramManager.containsHologram(name)) {
    ...
    public static void createHologramPages(ConvertorResult convertorResult, String name, Location location, List<List<String>> pages, HologramManager hologramManager) {
        if (hologramManager.containsHologram(name)) {
            Log.warn("A hologram with name '%s' already exists, skipping...", name);
            convertorResult.addSkipped();
            return;
