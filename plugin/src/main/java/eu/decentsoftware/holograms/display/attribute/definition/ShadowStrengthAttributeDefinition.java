...
public class ShadowStrengthAttributeDefinition implements AttributeDefinition<Float> {
    ...
    public void apply(AttributeValue<Float> value, FinalDisplayRenderState state) {
        ...
        if (finalValue != null) {
            state.addMetadata(BuiltInMetadataKeys.SHADOW_STRENGTH.createValue(finalValue));
        } else {
            state.addMetadata(BuiltInMetadataKeys.SHADOW_STRENGTH.createValue(getDefaultValue()));
        }
    }
